...
library(ggplot2)
p <- DimPlot(d1, reduction="umap",group.by = "seurat_clusters", split.by = "selfrenewal",label=T)
ggsave("figures/umap_s3s4.svg", plot = p , width = 12 ,height = 6) 
ggsave("figures/umap_s3s4.pdf", plot = p , width = 12, height = 6)

p <- DimPlot(d1, reduction="umap", group.by = "celltype", split.by = "selfrenewal",label=T)
...
a=VlnPlot(d[d$selfrenewal=="good",], features,group.by="seurat_clusters", stack = TRUE, sort = TRUE, flip = F) +
     theme(legend.position = "none") + ggtitle("Target Genes by Cluster")

b=VlnPlot(d[d$selfrenewal=="bad", features,group.by="seurat_clusters", stack = TRUE, sort = TRUE, flip = F) +
     theme(legend.position = "none") + ggtitle("Target Genes by Cluster")
...
# Subset good selfrenewal
a <- VlnPlot( subset(d, subset = selfrenewal == "high_selfrenewal"),
  features = features, group.by = "seurat_clusters", stack = TRUE, flip = FALSE
) + theme(legend.position = "none") + ggtitle("Good Repairment")

# Subset bad selfrenewal
b <- VlnPlot( subset(d, subset = selfrenewal == "low_selfrenewal"),
  features = features, group.by = "seurat_clusters", stack = TRUE, flip = FALSE
